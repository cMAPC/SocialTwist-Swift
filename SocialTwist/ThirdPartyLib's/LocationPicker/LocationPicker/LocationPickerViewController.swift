...
open class LocationPickerViewController: UIViewController {
    ...
    static public var completion: ((Location?) -> ())?
	...
	open override func viewDidLoad() {
        ...
        if #available(iOS 11.0, *) {
            navigationItem.title = "Search place"
    ...
    class func currentLocation() {
        ...
        let locationCoordinates = CLLocation(latitude: (manager.location?.coordinate.latitude)!, longitude: (manager.location?.coordinate.longitude)!)
        ...
        geocoder.cancelGeocode()
        geocoder.reverseGeocodeLocation(locationCoordinates) { response, error in
            if let error = error as NSError?, error.code != 10 { // ignore cancelGeocode errors
                ...
                location = Location(name: name, location: locationCoordinates, placemark: placemark)
